...
fn make_engine(rt: Handle) -> Engine {
    ...
    engine.register_fn("tool_input", |name: &str, frag: &str| {
        Expect::tool_input(name, frag)
    });
    engine.register_fn("tool_result", |s: &str| Expect::tool_result(s));
    engine.register_fn("tool_result_contains", |name: &str, frag: &str| {
        Expect::tool_result_contains(name, frag)
    });
    engine.register_fn("tool_result_error", |name: &str, is_error: bool| {
        Expect::tool_result_error(name, is_error)
    });
    engine.register_fn("text_regex", |pattern: &str| -> RhaiResult<Expect> {
        Expect::text_regex(pattern).map_err(to_rhai_err)
    });
    ...
    engine.register_fn("wait_tool_input", |name: &str, frag: &str| {
        Wait::tool_input(name, frag)
    });
    engine.register_fn("wait_tool_result_contains", |name: &str, frag: &str| {
        Wait::tool_result_contains(name, frag)
    });
    engine.register_fn("wait_tool_result_error", |name: &str, is_error: bool| {
        Wait::tool_result_error(name, is_error)
    });
    engine.register_fn("wait_text_regex", |pattern: &str| -> RhaiResult<Wait> {
        Wait::text_regex(pattern).map_err(to_rhai_err)
    });
