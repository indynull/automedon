...
impl Expect {
    ...
    /// Tool result whose stringified output contains `fragment`.
    pub fn tool_result_contains(name: impl Into<String>, fragment: impl Into<String>) -> Self {
        Self::new(Predicate::ToolResult {
            name: Some(name.into()),
            is_error: None,
            output_contains: Some(fragment.into()),
        })
    }

    /// Tool result with explicit error flag (and optional name).
    pub fn tool_result_error(name: impl Into<String>, is_error: bool) -> Self {
        Self::new(Predicate::ToolResult {
            name: Some(name.into()),
...
pub enum Predicate {
    ...
    ToolResult {
        ...
        /// Substring match against tool result `output` (empty means any output).
...
impl fmt::Display for Predicate {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Predicate::ToolResult {
                name,
                is_error,
                output_contains,
            } => write!(
                f,
                "tool result name={name:?} error={is_error:?} output~={output_contains:?}"
            ),
            ...
            Predicate::ToolResult {
                ...
                Event::ToolResult {
                    ...
                    name.as_ref().is_none_or(|want| want == n)
                        ...
                        && output_contains
                            .as_ref()
                            .is_none_or(|frag| output.contains(frag.as_str()))
                }
                _ => false,
            },
