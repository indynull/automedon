...
Automedon sits **outside** the agent. Your script (or Rust code) drives a **session**. Each turn, an **adapter** turns your prompt into a process spawn and maps stdout / ACP frames into a shared **event** stream. **Wait** and **expect** block on that stream until a condition matches.

```
Script / Rust
     |
     v
  Session -- prepare --> child product CLI
     |                        |
     |<---- events -----------+
     |
  Wait / Expect
